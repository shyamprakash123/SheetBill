...
export default function CreditNotes() {
  ...
  const { invoices, customers, initializeService } = useInvoiceStore()
  ...
  const handleCreateCreditNote = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNewCreditNote({ invoiceId: '', customerName: '', amount: '', reason: '', status: 'Draft' })
  ...
  const filteredCreditNotes = creditNotes.filter(note => {
    ...
    const matchesStatus = filterStatus === 'all' || note.status.toLowerCase() === filterStatus.toLowerCase()
  ...
  const stats = [
    ...
    },
    {
      ...
      icon: CalendarIcon,
      color: 'bg-yellow-500',
      change: `${creditNotes.filter(n => n.status === 'Issued').length} notes`
    },
    {
      ...
      icon: UserIcon,
      color: 'bg-green-500',
      change: `${creditNotes.filter(n => n.status === 'Applied').length} applied`
    }
  ]
          ...
          <form onSubmit={handleCreateCreditNote} className="space-y-6">
              ...
              >
                ...
                {invoices.map(invoice => (
                  <option key={invoice.id} value={invoice.id}>
                    {invoice.id} - {invoice.customerName} (₹{invoice.total.toLocaleString()})
                  </option>
                ))}
              </select>
            </div>
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setNewCreditNote({ ...newCreditNote, status: e.target.value as any })}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent"
              >
                <option value="Draft">Draft</option>
                <option value="Issued">Issued</option>
            ...
            <div className="flex justify-end space-x-3 pt-4">
              ...
              <Button 
                type="submit"
                loading={loading}
                disabled={!newCreditNote.invoiceId || !newCreditNote.amount || !newCreditNote.reason}
              >
                Create Credit Note
              </Button>
            </div>
