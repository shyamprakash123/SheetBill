...
export default function Inventory() {
  ...
  const getStockStatus = (stock: string | number) => {
    ...
    if (isNaN(stockNum)) return { color: 'bg-gray-100 text-gray-800', label: stock }
    if (stockNum === 0) return { color: 'bg-red-100 text-red-800', label: 'Out of Stock' }
    if (stockNum < 10) return { color: 'bg-yellow-100 text-yellow-800', label: 'Low Stock' }
