...
export class GoogleSheetsAPI {
  ...
  private async formatHeaders(
    ...
    for (const sheet of sheets) {
      ...
      try {
        ...
        console.warn(
          `Failed to format sheet ${sheet.properties.title}:`,
          error
        );
      }
    }
  ...
  async batchUpdateSheetData(
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(
        `Failed to batch update sheet data: ${
          error.error?.message || "Unknown error"
        }`
      );
    }
  ...
  async batchUpdate(
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(
        `Failed to batch update: ${error.error?.message || "Unknown error"}`
      );
    }
  ...
  async getSheetData(spreadsheetId: string, range: string) {
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(
          `Failed to get sheet data: ${error.error?.message || "Unknown error"}`
        );
      }
