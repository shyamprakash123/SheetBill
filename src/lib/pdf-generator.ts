...
import { Invoice } from "./backend-service";
          ...
          <div>
            ${
              this.companyInfo.logoUrl
                ? `<img src="${this.companyInfo.logoUrl}" alt="Logo" style="max-height: 60px; margin-bottom: 10px;">`
                : ""
            }
            <h1 style="color: #3b82f6; margin: 0; font-size: 24px;">${
              this.companyInfo.name
            }</h1>
            <p style="margin: 5px 0; color: #666;">${
              this.companyInfo.address
            }</p>
            <p style="margin: 5px 0; color: #666;">Phone: ${
              this.companyInfo.phone
            }</p>
            <p style="margin: 5px 0; color: #666;">Email: ${
              this.companyInfo.email
            }</p>
            <p style="margin: 5px 0; color: #666;">GSTIN: ${
              this.companyInfo.gstin
            }</p>
          </div>
          <div style="text-align: right;">
            <h2 style="color: #3b82f6; margin: 0; font-size: 28px;">INVOICE</h2>
            <p style="margin: 10px 0; font-size: 16px;"><strong>${
              invoice.id
            }</strong></p>
            <p style="margin: 5px 0; color: #666;">Date: ${format(
              new Date(invoice.date),
              "dd/MM/yyyy"
            )}</p>
            <p style="margin: 5px 0; color: #666;">Due Date: ${format(
          ...
          <div style="background: #f9fafb; padding: 15px; border-radius: 8px;">
            ...
            ${
              invoice.customerAddress
                ? `<p style="margin: 5px 0; color: #666;">${invoice.customerAddress}</p>`
                : ""
            }
            ${
              invoice.customerPhone
                ? `<p style="margin: 5px 0; color: #666;">Phone: ${invoice.customerPhone}</p>`
                : ""
            }
            ${
              invoice.customerEmail
                ? `<p style="margin: 5px 0; color: #666;">Email: ${invoice.customerEmail}</p>`
                : ""
            }
            ${
              invoice.customerGSTIN
                ? `<p style="margin: 5px 0; color: #666;">GSTIN: ${invoice.customerGSTIN}</p>`
                : ""
            }
          </div>
        </div>
          ...
          <tbody>
            ${invoice.items
              ...
              <tr style="background: ${index % 2 === 0 ? "#f9fafb" : "white"};">
                ...
                <td style="padding: 12px; text-align: right; border: 1px solid #ddd;">₹${item.rate.toFixed(
                  2
                )}</td>
                <td style="padding: 12px; text-align: right; border: 1px solid #ddd;">₹${item.amount.toFixed(
                  2
                )}</td>
                <td style="padding: 12px; text-align: right; border: 1px solid #ddd;">₹${item.taxAmount.toFixed(
                  2
                )}</td>
...
Please find attached your invoice ${invoice.id} for ₹${invoice.total.toFixed(
      2
    )}.
