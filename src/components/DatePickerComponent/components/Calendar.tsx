...
const Calendar = ({
  ...
  return (
    <DayPicker
      ...
      selected={previewRange || selected}
      ...
      classNames={{
        ...
        day: cx(
          "w-full p-1.5  text-sm focus:z-10",
          "text-gray-900 dark:text-gray-50",
          "hover:text-white",
          ...
          `${
            mode !== "single" &&
            (isBefore(selected?.from, hoveredDate)
              ? "hover:!rounded-r-md"
              : isAfter(selected?.from, hoveredDate)
              ? "hover:!rounded-l-md !rounded-r-none"
              : "hover:!rounded-md")
          } ${
            mode !== "single" &&
            (previewRange?.from || selected?.from
              ? "hover:rounded-r-md"
              : "hover:!rounded-md")
          }`,
          `${mode === "single" && "!rounded-md"}`
        ),
        day_today:
          "font-semibold !rounded-md border-2 border-blue-500 dark:ring-blue-500",
        day_selected: cx(
          "rounded-md",
      ...
      components={{
        ...
        Day: ({ date, displayMonth }: DayProps) => {
          ...
          const { selected, today, disabled, range_middle, range_end } =
            activeModifiers;
          ...
          const isInPreviewRange =
            ...
            !selectedTo &&
            hoveredDate &&
            ((isBefore(date, hoveredDate) && isAfter(date, selectedFrom)) ||
              (isAfter(date, hoveredDate) && isBefore(date, selectedFrom)) ||
              isSameDay(date, hoveredDate));

          // console.log("isInPreviewRange", isInPreviewRange);
