...
import * as PopoverPremitive from "@radix-ui/react-popover";
import {
  useDateSegment,
  useTimeField,
...
const Trigger = React.forwardRef<HTMLButtonElement, TriggerProps>(
  (
    ...
    return (
      <PopoverPremitive.Trigger asChild>
        <button
          ref={forwardedRef}
          className={cx(triggerStyles({ hasError }), className, "min-w-64")}
          {...props}
        >
...
//#region PopoverPremitive
// ============================================================================

const CalendarPopover = React.forwardRef<
  React.ElementRef<typeof PopoverPremitive.Content>,
  React.ComponentProps<typeof PopoverPremitive.Content>
>(({ align, className, children, ...props }, forwardedRef) => {
  return (
    <PopoverPremitive.Portal>
      <PopoverPremitive.Content
        forceMount
        ref={forwardedRef}
        sideOffset={10}
        side="bottom"
        ...
        className={cx(
          ...
          "data-[state=closed]:animate-roll-up-and-fade",
          "data-[state=open]:data-[side=bottom]:animate-roll-down-and-fade data-[state=open]:data-[side=left]:animate-slide-left-and-fade data-[state=open]:data-[side=right]:animate-slide-right-and-fade data-[state=open]:data-[side=top]:animate-slide-up-and-fade",
          className
        )}
        {...props}
      >
...
const PresetContainer = <TPreset extends Preset, TValue>({
  ...
  currentValue,
  // Event handler when a preset is hovered
  onHover,
  // Event handler when hover ends
  onHoverEnd,
  ...
  const handleHover = (preset: TPreset) => {
    if (isDateRangePresets(preset)) {
      onHover(preset.dateRange as TValue);
    } else if (isDatePresets(preset)) {
      onHover(preset.date as TValue);
    }
  };
  ...
  return (
    ...
    >
      <Trigger
        ...
        disabled={disabled}
        className={cx(className, "size-12")}
        hasError={hasError}
...
const RangeDatePicker = ({
  ...
  const onPreviewRangeChange = (range: DateRange | undefined) => {
  ...
  const displayRange = React.useMemo(() => {
    if (previewRange) {
      return `${
        previewRange.from
          ? formatDate(previewRange.from, locale, showTimePicker)
          : ""
      } - ${
        previewRange.to
          ? formatDate(previewRange.to, locale, showTimePicker)
          : ""
      }`;
    } else if (range) {
      return `${
    ...
    return null;
  }, [range, previewRange, locale, showTimePicker]);
          ...
          <div className="flex flex-col overflow-x-auto sm:flex-row sm:items-start">
              ...
              >
                <div className="absolute px-3 sm:inset-0 sm:left-0 sm:p-2">
                  <PresetContainer
                    ...
                    onHover={onPreviewRangeChange}
                    onHoverEnd={() => onPreviewRangeChange(undefined)}
                  />
                </div>
              </div>
            )}
            <div className="overflow-x-auto">
              <CalendarPrimitive
                ...
                onPreviewSelect={setPreviewRange}
                month={month}
                onMonthChange={setMonth}
                numberOfMonths={2}
                disabled={disabledDays}
